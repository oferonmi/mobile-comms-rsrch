...
class MimoOfdmE2ESystemRLTraining(Model):
    ...
    def call(self, batch_size, ebno_db, perturbation_variance=tf.constant(0.0, tf.float32)):
        ...
        x_rg = self._rg_mapper(x_p) # added for OFDM
        ...
        if self._training:
            ...
            #c = tf.reshape(c, [-1, num_ut, resource_grid.num_streams_per_tx, n])
            ...
            #rx_loss = tf.reduce_mean(bce, axis=2) # Average over the bits mapped to the same baseband symbol
            rx_loss = tf.reduce_mean(bce)
            print("rx_loss shape: ", rx_loss.shape)
            print("rx_loss value: ", rx_loss)
